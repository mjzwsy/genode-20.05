...
static void fb_cvt_print_name(struct fb_cvt_data *cvt)
{
	...
	read = sprintf(buf+offset, "fbcvt: %dx%d@%d: CVT Name - ",
			cvt->xres, cvt->yres, cvt->refresh);
	offset += read;
	cnt -= read;

	if (cvt->status)
		sprintf(buf+offset,"Not a CVT standard - %d.%03d Mega "
			 "Pixel Image\n", pixcount, pixcount_mod);
	else {
		if (pixcount) {
			read = sprintf(buf+offset, "%d", pixcount);
			cnt -= read;
			offset += read;
		}

		read = sprintf(buf+offset, ".%03dM", pixcount_mod);
		cnt -= read;
		offset += read;

		if (cvt->aspect_ratio == 0)
			read = sprintf(buf+offset, "3");
		else if (cvt->aspect_ratio == 3)
			read = sprintf(buf+offset, "4");
		else if (cvt->aspect_ratio == 1 || cvt->aspect_ratio == 4)
			read = sprintf(buf+offset, "9");
		else if (cvt->aspect_ratio == 2)
			read = sprintf(buf+offset, "A");
		else
			read = 0;
		cnt -= read;
		offset += read;
